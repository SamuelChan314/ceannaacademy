...
const els = {
  ...
  duplicateTrip: document.querySelector("#duplicateTrip"),
...
function duplicateActiveTrip() {
  resetDeleteConfirmation();
  saveTrip();
  const source = state.trips.find((trip) => trip.id === state.activeTripId) || currentTripSnapshot();
  const copy = normalizeTrip({
    ...deepClone(source),
    id: crypto.randomUUID(),
    tripName: `${source.tripName || "Untitled Trip"} Copy`,
...
function renderItems() {
  ...
  els.itemsList.innerHTML = items.map((item) => `
    <article class="trip-item" draggable="${els.sortBy.value === "manual"}" data-id="${item.id}">
      ...
      <div class="item-actions" aria-label="Item actions">
        <button class="icon-button" type="button" aria-label="Duplicate ${escapeHtml(item.title)}" data-duplicate="${item.id}">Copy</button>
        <button class="icon-button" type="button" aria-label="Edit ${escapeHtml(item.title)}" data-edit="${item.id}">Edit</button>
        <button class="icon-button danger" type="button" aria-label="Remove ${escapeHtml(item.title)}" data-remove="${item.id}">x</button>
      </div>
    </article>
  `).join("");
}
...
function duplicateItem(id) {
  ...
  const copy = {
    ...deepClone(source),
    id: crypto.randomUUID(),
    title: `${source.title || "Plan item"} Copy`,
  };
...
function exportPdf() {
  downloadBlob(makePosterPdf(), fileBaseName("travel-planner", "pdf"), "application/pdf");
}
...
function parsePdfLine(line) {
  if (line.includes("|")) {
    return line
      .split(/(?=\b(?:Flight|Stay|Food|Transport|Activity|Shopping|Other)\s*\|)/i)
      .map((part) => part.split("|").map((cell) => cell.trim()))
      .filter((row) => categories.includes(row[0]) && row.length >= 4);
  }
...
function rowsToItems(rows) {
  ...
  const headerIndex = normalizedRows.findIndex(isItineraryHeader);
  const headers = headerIndex >= 0 ? normalizedRows[headerIndex].map(normalizeHeader) : [];
  const hasHeaders = headerIndex >= 0;
  const dataRows = itineraryRowsAfterHeader(normalizedRows, headerIndex);
...
function isItineraryHeader(row) {
  const headers = row.map(normalizeHeader);
  const hasCategory = headers.includes("category") || headers.includes("type");
  const hasItem = headers.includes("item") || headers.includes("title") || headers.includes("plan") || headers.includes("name");
  const hasCost = headers.includes("cost") || headers.includes("amount") || headers.includes("price");
  return hasCategory && hasItem && hasCost;
}

function itineraryRowsAfterHeader(rows, headerIndex) {
  const start = headerIndex >= 0 ? headerIndex + 1 : 0;
  const output = [];
  for (const row of rows.slice(start)) {
    const meaningful = row.filter(Boolean);
    if (!meaningful.length) {
      if (headerIndex >= 0 && output.length) break;
      continue;
    }
    if (headerIndex >= 0 && /^notes?$/i.test(row[0] || "")) break;
    output.push(row);
  }
  return output.filter((row) => row.filter(Boolean).length >= 2);
}

function makePosterPdf() {
  const width = 612;
  const height = 792;
  const total = totals();
  const budget = Number(els.budget.value || 0);
  const byCategory = categoryTotals();
  const itinerary = [...state.items].sort((a, b) => dayRank(a) - dayRank(b) || timeRank(a).localeCompare(timeRank(b)));
  const pages = [];
  let commands = [];
  let y = 0;

  const add = (command) => commands.push(command);
  const rect = (x, yPos, w, h, color) => add(`${pdfColor(color)} ${x} ${yPos} ${w} ${h} re f`);
  const strokeRect = (x, yPos, w, h, color = "#ffffff", lineWidth = 1) => add(`${pdfStroke(color)} ${lineWidth} w ${x} ${yPos} ${w} ${h} re S`);
  const line = (x1, y1, x2, y2, color = "#ffffff", lineWidth = 1) => add(`${pdfStroke(color)} ${lineWidth} w ${x1} ${y1} m ${x2} ${y2} l S`);
  const text = (value, x, y, size = 12, color = "#11131a", font = "F1") => {
    add(`BT /${font} ${size} Tf ${pdfColor(color)} ${x} ${y} Td (${escapePdf(value)}) Tj ET`);
  };
  const wrapped = (value, x, y, maxChars, size = 10, color = "#11131a", font = "F1", leading = 13, maxLines = 3) => {
    const lines = wrapPdfText(String(value || ""), maxChars).slice(0, maxLines);
    lines.forEach((entry, index) => text(entry, x, y - index * leading, size, color, font));
  ...
  const startPage = (pageNumber) => {
    commands = [];
    pages.push(commands);
    rect(0, 0, width, height, "#f7f3ea");
    rect(0, 718, width, 74, "#101217");
    rect(38, 734, 96, 34, "#70d6b7");
    text("CEANNA", 54, 755, 12, "#101217", "F2");
    text("ACADEMY", 54, 741, 12, "#101217", "F2");
    text(els.tripName.value || "Travel Planner", 154, 752, 24, "#f7f3ea", "F2");
    wrapped(tripMetaText(), 154, 729, 62, 9, "#b7bdc8", "F1", 11, 2);
    if (pageNumber > 1) text(`Itinerary continued / Page ${pageNumber}`, 430, 704, 9, "#6a6f78", "F1");
    y = 684;
  };

  const footer = (pageNumber) => {
    line(38, 32, 574, 32, "#d7d2c6", 0.8);
    text(`Ceanna Academy Travel Planner / Page ${pageNumber}`, 38, 18, 8, "#6a6f78", "F1");
  };
  ...
  rect(38, 620, 164, 46, "#ffffff");
  rect(224, 620, 164, 46, "#ffffff");
  rect(410, 620, 164, 46, "#ffffff");
  strokeRect(38, 620, 164, 46, "#e4e1d8", 0.6);
  strokeRect(224, 620, 164, 46, "#e4e1d8", 0.6);
  strokeRect(410, 620, 164, 46, "#e4e1d8", 0.6);
  text("TOTAL COST", 52, 650, 8, "#6a6f78", "F2");
  text(money(total), 52, 630, 16, "#101217", "F2");
  text("BUDGET USED", 238, 650, 8, "#6a6f78", "F2");
  text(budget ? `${Math.round((total / budget) * 100)}%` : "0%", 238, 630, 16, "#101217", "F2");
  text("TRIP DAYS", 424, 650, 8, "#6a6f78", "F2");
  text(String(daysPlanned()), 424, 630, 16, "#101217", "F2");

  y = 588;
  if (Object.keys(byCategory).length) {
    text("Category Spend", 38, y, 14, "#101217", "F2");
    y -= 18;
    Object.entries(byCategory).slice(0, 7).forEach(([category, value]) => {
      const percent = total ? Math.max(0.04, value / total) : 0;
      text(category, 38, y, 9, "#101217", "F2");
      rect(122, y - 4, 300, 7, "#e4e1d8");
      rect(122, y - 4, Math.round(300 * percent), 7, categoryColors[category] || categoryColors.Other);
      text(money(value), 438, y, 9, "#101217", "F1");
      y -= 17;
    });
    y -= 14;
  }

  text("Itinerary", 38, y, 18, "#101217", "F2");
  y -= 14;
  line(38, y, 574, y, "#101217", 1);
  y -= 20;

  itinerary.forEach((item, index) => {
    const titleLines = wrapPdfText(item.title || "Plan item", 58).slice(0, 2);
    const noteLines = item.note ? wrapPdfText(stripMarkdownLinks(item.note), 78).slice(0, 2) : [];
    const cardHeight = 58 + Math.max(0, titleLines.length - 1) * 13 + noteLines.length * 10;
    ensureSpace(cardHeight + 12);

    const cardBottom = y - cardHeight;
    const chipColor = categoryColors[item.category] || categoryColors.Other;
    rect(38, cardBottom, 536, cardHeight, "#ffffff");
    strokeRect(38, cardBottom, 536, cardHeight, "#e4e1d8", 0.6);
    rect(38, cardBottom, 6, cardHeight, chipColor);
    rect(54, y - 24, 76, 18, chipColor);
    text(item.category, 62, y - 18, 8, "#101217", "F2");
    text(dayLabel(item.days) || `Item ${index + 1}`, 144, y - 12, 10, "#ff6b5f", "F2");
    text([item.city, timeLabel(item)].filter(Boolean).join(" | "), 226, y - 12, 9, "#6a6f78", "F1");
    text(money(item.cost), 516, y - 12, 9, "#101217", "F2");
    titleLines.forEach((lineText, lineIndex) => text(lineText, 54, y - 40 - lineIndex * 13, 12, "#101217", "F2"));
    noteLines.forEach((lineText, lineIndex) => text(lineText, 54, y - 65 - lineIndex * 10, 8, "#6a6f78", "F1"));
    y = cardBottom - 12;
  });

  if (els.notes.value) {
    ensureSpace(78);
    text("Trip Notes", 38, y, 14, "#101217", "F2");
    y -= 18;
    rect(38, y - 54, 536, 62, "#ffffff");
    strokeRect(38, y - 54, 536, 62, "#e4e1d8", 0.6);
    wrapped(els.notes.value, 52, y - 10, 92, 9, "#101217", "F1", 11, 4);
  }

  footer(pages.length);

  // Keep a machine-readable itinerary block for importing app-created PDFs.
  text("Category | City | Days | Item | Cost | From Time | To Time | Note", 900, 900, 1, "#f7f3ea", "F1");
  state.items.forEach((item, index) => {
    text([item.category, item.city || "", dayLabel(item.days), item.title, money(item.cost), item.fromTime || "", item.toTime || "", item.note || ""].join(" | "), 900, 890 - index * 2, 1, "#f7f3ea", "F1");
  });
  ...
  const objects = Array(nextObjectId - 1);

  objects[0] = "<< /Type /Catalog /Pages 2 0 R >>";
  objects[1] = `<< /Type /Pages /Kids [${pageIds.map((id) => `${id} 0 R`).join(" ")}] /Count ${pages.length} >>`;
  pages.forEach((pageCommands, index) => {
    const content = pageCommands.join("\n");
    objects[pageIds[index] - 1] = `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${width} ${height}] /Resources << /Font << /F1 ${fontRegularId} 0 R /F2 ${fontBoldId} 0 R >> >> /Contents ${contentIds[index]} 0 R >>`;
    objects[contentIds[index] - 1] = `<< /Length ${content.length} >>\nstream\n${content}\nendstream`;
  });
...
els.duplicateTrip.addEventListener("click", duplicateActiveTrip);
...
els.itemsList.addEventListener("click", (event) => {
  ...
  if (duplicateButton) duplicateItem(duplicateButton.dataset.duplicate);
