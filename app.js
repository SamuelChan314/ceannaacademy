...
function makePosterPdf() {
  ...
  const rect = (x, y, w, h, color) => add(`${pdfColor(color)} ${x} ${y} ${w} ${h} re f`);
  const strokeRect = (x, y, w, h, color = "#ffffff", lineWidth = 1) => add(`${pdfStroke(color)} ${lineWidth} w ${x} ${y} ${w} ${h} re S`);
  ...
  rect(0, 0, width, 792, "#f7f3ea");
  rect(0, 642, width, 150, "#101217");
  rect(0, 642, 210, 150, "#70d6b7");
  rect(210, 642, 170, 150, "#f3c969");
  rect(380, 642, 232, 150, "#ff6b5f");

  text("CEANNA ACADEMY", 38, 744, 11, "#101217", "F2");
  wrapped(els.tripName.value || "Travel Planner", 38, 704, 24, 32, "#101217", "F2", 34, 2);
  text("TRAVEL PLANNER", 420, 744, 12, "#f7f3ea", "F2");
  wrapped(tripMetaText(), 420, 718, 28, 11, "#f7f3ea", "F1", 14, 3);

  rect(38, 570, 166, 48, "#ffffff");
  rect(223, 570, 166, 48, "#ffffff");
  rect(408, 570, 166, 48, "#ffffff");
  text("TOTAL COST", 54, 601, 8, "#6a6f78", "F2");
  text(money(total), 54, 580, 18, "#101217", "F2");
  text("BUDGET USED", 239, 601, 8, "#6a6f78", "F2");
  text(budget ? `${Math.round((total / budget) * 100)}%` : "0%", 239, 580, 18, "#101217", "F2");
  text("DAYS", 424, 601, 8, "#6a6f78", "F2");
  text(String(daysPlanned()), 424, 580, 18, "#101217", "F2");

  text("Category Spend", 38, 535, 15, "#101217", "F2");
  let categoryY = 512;
  Object.entries(byCategory).slice(0, 6).forEach(([category, value]) => {
    const percent = total ? Math.max(0.04, value / total) : 0;
    text(category, 38, categoryY, 9, "#101217", "F2");
    rect(124, categoryY - 4, 260, 7, "#e4e1d8");
    rect(124, categoryY - 4, Math.round(260 * percent), 7, categoryColors[category] || categoryColors.Other);
    text(money(value), 398, categoryY, 9, "#101217", "F1");
    categoryY -= 18;
  });

  text("Itinerary Highlights", 38, 384, 18, "#101217", "F2");
  line(38, 372, 574, 372, "#101217", 1.2);

  let itemY = 344;
  const visibleItems = itinerary.slice(0, 9);
  visibleItems.forEach((item, index) => {
    const chipColor = categoryColors[item.category] || categoryColors.Other;
    rect(38, itemY - 10, 72, 18, chipColor);
    text(item.category, 46, itemY - 4, 8, "#101217", "F2");
    text(dayLabel(item.days) || `Item ${index + 1}`, 124, itemY, 10, "#ff6b5f", "F2");
    text([item.city, timeLabel(item)].filter(Boolean).join(" | "), 205, itemY, 9, "#6a6f78", "F1");
    text(money(item.cost), 512, itemY, 9, "#101217", "F2");
    wrapped(item.title, 124, itemY - 18, 68, 13, "#101217", "F2", 15, 2);
    if (item.note) wrapped(stripMarkdownLinks(item.note), 124, itemY - 48, 74, 8, "#6a6f78", "F1", 10, 2);
    itemY -= item.note ? 78 : 56;
  });

  if (itinerary.length > visibleItems.length) {
    text(`+ ${itinerary.length - visibleItems.length} more itinerary item(s) in the planner`, 38, 42, 10, "#6a6f78", "F2");
  }

  text("Generated by Ceanna Academy Travel Planner", 38, 24, 8, "#6a6f78", "F1");
