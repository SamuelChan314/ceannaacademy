...
function deleteActiveTrip() {
  if (state.trips.length <= 1) {
    alert("Keep at least one trip tab. Create a new trip before deleting this one.");
    return;
  }

  const currentName = els.tripName.value.trim() || "this trip";
  const confirmed = confirm(`Delete "${currentName}"? This removes the saved trip from this browser.`);
  if (!confirmed) return;

  const deleteIndex = state.trips.findIndex((trip) => trip.id === state.activeTripId);
